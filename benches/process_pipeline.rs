...
use procwarden::common::{ProcessInfo, SortColumn, ProcessListState};
use procwarden::process_list::{sort_processes, filter_processes, format_memory};
...
fn bench_filter(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000] {
        let procs = make_procs(size);
        group.bench_with_input(BenchmarkId::new("hide_self_off", size), &procs, |b, procs| {
            b.iter(|| filter_processes(black_box(procs.clone()), false, ""))
        });
        group.bench_with_input(BenchmarkId::new("hide_self_on", size), &procs, |b, procs| {
            b.iter(|| filter_processes(black_box(procs.clone()), true, ""))
        });
    }
    group.finish();
}
...
fn bench_full_pipeline(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000] {
        let procs = make_procs(size);
        group.bench_with_input(BenchmarkId::new("sort_filter_format", size), &procs, |b, procs| {
            let mut state = ProcessListState::new();
            b.iter(|| {
